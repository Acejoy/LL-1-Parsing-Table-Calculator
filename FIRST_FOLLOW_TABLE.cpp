        ...
        for(auto r: rules){
            ...
            for(auto seg: seglist){
                ...
                if(terminals.find(seg)!=terminals.end()){
                    ...
                    if(done[seg]==true){            //checking if the first of the token seg is calculated already or else it is calculated
                        first_set_prods=FIRST[seg];

                    }
                    else{
                        //cout<<"calling the first funct for "<<seg<<" from "<<s<<endl;   
                    ...
                    for(auto c: FIRST[seg]){
                        if(c!="e"){                                 //inserting all elements of first(seg) except e
                            FIRST[s].insert(c);
                        }                            
                    }
...
set<string> calc_follow(string s,map<string,bool> &done){
    ...
    for(auto p: prod_rules){
        ...
        for(auto rules: p.second){
            ...
            if(it==seglist.end()){                  //if not found continue to next rule
                continue;
            }
            //cout<<s<<"-->"<<non_term_var<<"="<<rules<<endl;
            ...
            if(it==seglist.end()){
                ...
                if(done[non_term_var]==true){
                    //cout<<"%hello\n";
                    //cout<<"\n2The follow of "<<non_term_var<<" is entered in "<<s<<endl;;
                    temp=FOLLOW[non_term_var];
                }
                else{
                    //cout<<"&hello\n";
                    //cout<<"\n2calling the follow funct for "<<non_term_var<<" from "<<s<<endl;   
        ...
        for(auto rule:p.second){
            ...
            if(terminals.find(first_token)!=terminals.end()){
                ...
                for(auto a:temp_first){
                    if(a!="e"){                             //inserting all elements except e
...
int main(){
   ...
   map<string,bool> calced;
    ...
    for(auto c: non_terminals){
        if(calced[c]==false){
            //cout<<"\n enetring into funct the non term "<<c<<endl;
            calc_first(c,calced);
        }
